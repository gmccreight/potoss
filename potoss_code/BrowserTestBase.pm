...
sub runtests {
    ...
    # exactly what it outputs here.  Including the $sel-> part.
    # If we're using WWW::Mechanize for our testing, then we'll need to
    # map the Selenium tests to comparable "mech" tests.  If we're using
    # Selenium, then just go ahead and use it.
    
    my $sel = ($self->{TYPE} eq 'mech')
        ? $self             # The mech wrappers in this package
        : $self->{BROWSER}; # The selenium object
